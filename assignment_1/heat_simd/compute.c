...
double compute_left_gridpoint(const struct parameters *p, double *matrix, size_t num_colls, size_t index)
{
	...
	double retained_temp = (p->conductivity[index] * matrix[index]);
	double new_temp = 0;

    const double dirW  = (1/4) * (sqrt(2)/(sqrt(2)+1));
    const double diaW  = (1/4) * (1/(sqrt(2)+1))

    
    _m256d conducted_temp_dir = _mm256_set_pd(matrix[top_middle_index], matrix[middle_left_index], matrix[middle_right_index], matrix[bottom_middle_index]);
    _m256d conducted_temp_dia = _mm256_set_pd(matrix[top_left_index], matrix[top_right_index], matrix[bottom_left_index], matrix[bottom_right_index]);

    
    _md256 weighted_conducted_dir = _mm256_mul_pd(dirW, conducted_temp_dir); 
    _m256d weighted_conducted_dia = _mm256_mul_pd(diaW, conducted_temp_dia);
